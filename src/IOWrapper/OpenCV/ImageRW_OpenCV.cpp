...
{
	cv::Mat m = cv::imread(filename, 0/*CV_LOAD_IMAGE_GRAYSCALE*/);
	if(m.rows*m.cols==0)
	{
...
{
	cv::Mat m = cv::imread(filename, 1/*CV_LOAD_IMAGE_COLOR*/);
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imread(filename, -1/*CV_LOAD_IMAGE_UNCHANGED*/);
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imdecode(cv::Mat(numBytes,1,CV_8U, data), 0/*CV_LOAD_IMAGE_GRAYSCALE*/);
	if(m.rows*m.cols==0)
	{
